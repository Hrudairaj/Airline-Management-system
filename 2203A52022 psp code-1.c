...
A C PROGRAM TO RESERVE OR CANCEL AN AIRLINE TICKETS WITHIN MINUTES 
*/
...
struct kingfisher_airline
{
	...
	int seat_num;
	char email[15];
	struct kingfisher_airline *following;
}
*begin, *stream;
struct kingfisher_airline *dummy;
...
void main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   WELCOME TO KINGFISHER AIRLINES                   ");
		printf("\n\t\t   *******************************************************************");
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct mufti_airline*)malloc(sizeof(struct kingfisher_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct kingfisher_airline *)malloc(sizeof(struct kingfisher_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	...
	printf("\n\n\t Details have been saved to a file (Kingfisher records)");
	fclose(fpointer);
}
